...
class PatchMatch(object):
    ...
    PatchMatch class
    """
    def __init__(self, A, B, A_, B_, patch_size):
        ...
        self.A_ = self.tensor_to_numpy(A_)
        self.B_ = self.tensor_to_numpy(B_)
    ...
    def nnf_init(self):
        ...
        """
        h, w, c = self.A.shape
        self.nnf[:, :, 0] = np.random.randint(h, size=(h, w))
        self.nnf[:, :, 1] = np.random.randint(w, size=(h, w))
        for i in range(h):
            for j in range(w):
                x, y = self.nnf[i, j]
    ...
    def cal_dist(self, i, j, x, y):
        ...
        patch_B_ = self.B_[x - dxu: x + dxd, y - dyl: y + dyr]
        patch_A_ = self.A_[i - dxu: i + dxd, j - dyl: j + dyr]
        patch_B = self.B[x - dxu: x + dxd, y - dyl: y + dyr]
        return (np.sum((patch_A - patch_B_) ** 2) + np.sum((patch_A_ - patch_B) ** 2)) / ((dxu + dxd) * (dyl + dyr))
    ...
    def propagate_pixel(self, i, j, di, dj, search_radius):
        ...
        if 0 <= i - di < h:  # check if the upper pixel is within the image
            x_up, y_up = self.nnf[i - di, j]
            x_up += di
            if 0 <= x_up < h and 0 <= y_up < w:
                dist_up = self.cal_dist(i, j, x_up, y_up)
                if dist_up < best_dist:
                    best_dist = dist_up
                    x_nn, y_nn = x_up, y_up
        if 0 <= j - dj < w:  # check if the left pixel is within the image
            x_left, y_left = self.nnf[i, j - dj]
            y_left += dj
            if 0 <= x_left < h and 0 <= y_left < w:
                dist_left = self.cal_dist(i, j, x_left, y_left)
                if dist_left < best_dist:
                    best_dist = dist_left
                    x_nn, y_nn = x_left, y_left
        # random search
        r = search_radius if search_radius else max(h, w)
        while r >= 1:
            x_rand = random.randint(max(0, i - r), min(h - 1, i + r))
            y_rand = random.randint(max(0, j - r), min(w - 1, j + r))
